...
global {
	...
	init {
		...
		list<building> industrial_buildings <- building  where (each.type="Trabajo") ;
		list<building> recreative_buildings <- building where (each.type="Ocio");
