...
species road  {
	...
	rgb color <- #black ;
	list local;
	int habitantes <- length(agents_inside(self))-1 update: length(agents_inside(self))-1;
	reflex actualizar{
		habitantes <- length(agents_inside(self))-1;
	}
	/*We define their aspect */
	aspect base {
		/*Their shape will be adjusted to the one in the shape_file and their color will be black */
...
experiment basic_model type: gui {
	...
	output {
		...
		display chart_display refresh: every(1#cycles) {
			...
			chart "Individuals on a street" type:pie background: #lightgray size:{1, 0.33} position:{0, 0.66}{
				datalist (distribution_of(road collect each.habitantes,20,0,10) at "legend") value:(distribution_of(road collect each.habitantes,20,0,10) at "values");
			}
		}
